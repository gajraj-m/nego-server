...
function calculateOverallSimilarity(order1, order2) {
  ...
  const paymentSimilarity =
    order1.payment_collector == order2.payment_collector ? 1 : 0;
  const settlementSimilarity =
    order1.settlement_basis == order2.settlement_basis ? 1 : 0;

  // Calculate overall similarity considering weights for different attribute types
  const overallSimilarity =
    0.5 * numericalSimilarity +
    0.25 * paymentSimilarity +
    0.25 * settlementSimilarity;
